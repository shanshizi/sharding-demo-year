...
public class AutoConfigDateNodes implements ApplicationRunner {
	...
	private final static Integer Year = 2022;
	...
	private ShardingTableRuleConfiguration createTableRule(
		...
		try {
			...
			// 执行得物理表的配置结果
			actualDataNodes = (String) buildNodes.invoke(o, logicTable, Year);

			// 执行建表逻辑
			Method createTables = aClass.getMethod("createTables", ShardingSphereDataSource.class, String.class,
					Integer.class);

			createTables.invoke(o, (ShardingSphereDataSource) dataSource, logicTable, Count);
		}
		catch (Exception e) {
